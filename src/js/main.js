...
function ReceiveUpdatedDoc(doc) {
  ...
  for (var i = 0; i < caller.signals.length; i++) {
    ...
    if (signal.signal_type === "new_ice_candidate") {
      ...
      peerConnection2.addIceCandidate(new RTCIceCandidate(signal.candidate), onAddIceCandidateSuccess, onAddIceCandidateError);
    } 
    else if (signal.signal_type === "new_description") {
      log("PC2 is setting remote description");
      peerConnection2.setRemoteDescription(new RTCSessionDescription(signal.sdp), function () {
